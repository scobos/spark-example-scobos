...
public class MockitoTests {
	...
	public void MockListSize() {
		List list = mock(List.class); // un mock va a ser un suplantador de la clase list
	...
	public void MockListSizeWithMultipleReturnValues() {
		...
		verify(list, times(3)).size(); //sobre el mock list se ha llamado 3 veces a su método size
	}

	@Test
	public void MockListGet() {
		List<String> list = mock(List.class);
		when(list.get(0)).thenReturn("Hello World"); //se ha llamado al metodo get con un 0 de argumento
	...
	public void MockWithUnspecifiedArguments(){
		...
		verify(c, atLeastOnce()).compareTo(5);  //al menos una vez
		verify(c, atLeast(1)).compareTo(5); //al menos N veces
	}
